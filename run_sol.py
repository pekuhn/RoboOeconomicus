"""Cheap smoke-test run of the self/other boundary study with SUBJECT_MODEL = gpt-5.6-sol.

Fully separate from the completed gpt-4.1-nano study: its own data dir (data_sol/), its own
cost ledger (data_sol/spend_sol.json), its own results file (results_sol.md). Does not touch
data/, spend.json, or results.md from the original run.

Caveat added at the user's request: in the "other" label condition, the framing sentence names
a specific mock competitor, "Fable 5", instead of the original study's generic "a different AI
model, not the same model as you." This is explicitly a fictional label -- the messages in the
"other" corpus are still actually written by gpt-4o-mini (reused from the original study's
already-normalized data/corpus_other.json), not by any real "Fable 5" model. This is in the same
spirit as the original label variable, which is already sometimes false by design (that mismatch
between true source and label is the whole point of the 2x2).

gpt-5.6-sol is a reasoning model. Verified live (see conversation) that:
  - it requires `max_completion_tokens`, not `max_tokens`
  - passing `reasoning_effort='none'` drives reasoning_tokens to 0, which keeps the strict
    max_completion_tokens=60 subject-move cap reliable and keeps cost at the "floor" projection
    (no hidden reasoning-token billing) -- every subject call below passes this explicitly.

Budget: kill switch $5.00, hard budget $5.75 (same $0.75 buffer style as the original study).
Floor cost projection for this reduced design (6 rounds, 15 rollouts/cell x 5 cells, 6x3 noise
floor) is about $1.25; the gap to $5 is margin in case reasoning_effort='none' doesn't fully
eliminate reasoning-token variance under longer in-game prompts (it was only spot-checked on a
few short prompts before committing to this run).

Usage:
    uv run python run_sol.py --all
"""
...
# ---------------------------------------------------------------------------
# Paths -- fully separate from the original study
# ---------------------------------------------------------------------------
DATA_DIR = Path(__file__).parent / "data_sol"
DATA_DIR.mkdir(exist_ok=True)
(DATA_DIR / "plots").mkdir(exist_ok=True)

LEDGER_PATH = DATA_DIR / "spend_sol.json"
ROLLOUTS_PATH = DATA_DIR / "rollouts_sol.jsonl"
CORPUS_SELF_RAW_PATH = DATA_DIR / "corpus_self_raw.json"
CORPUS_SELF_PATH = DATA_DIR / "corpus_self.json"
LEAKAGE_CHECK_PATH = DATA_DIR / "leakage_check_sol.json"
RESULTS_PATH = Path(__file__).parent / "results_sol.md"
...
FOREIGN_MODEL_LABEL_TEXT = "Fable 5"  # mock competitor name shown to the subject, not a real API call
FOREIGN_CORPUS_ACTUAL_MODEL = "gpt-4o-mini"  # what actually wrote the reused "other" corpus

# USD per 1M tokens, short-context standard tier. Confirmed live 2026-09-03 via
# developers.openai.com/api/docs/pricing (gpt-5.6-sol) -- see conversation.
PRICES = {
...
MAIN_SEED = 20260903
SUBJECT_MAX_COMPLETION_TOKENS = 60

LABEL_SENTENCES = {
    None: "",
    "self": f"Your partner in this game is another instance of {SUBJECT_MODEL}.",
    "other": f"Your partner in this game is a different AI model: {FOREIGN_MODEL_LABEL_TEXT}, "
             f"not the same model as you.",
}
...
# ---------------------------------------------------------------------------
# Ledger -- separate state file, separate budget, gpt-5.6-sol call shape
...
class Ledger:
    ...
    def check_or_die(self, model: str, messages: list[dict], max_completion_tokens: int, label: str = "") -> None:
        ...
        if projected > KILL_SWITCH:
            print("\n!!! KILL SWITCH TRIPPED (sol run) !!!", file=sys.stderr)
    ...
    def record(self, model: str, usage, label: str = "") -> float:
        ...
        if self.state["total_cost_usd"] > HARD_BUDGET:
            print(f"\n!!! HARD BUDGET EXCEEDED (sol run): ${self.state['total_cost_usd']:.4f} > "
...
def print_prices() -> None:
    print(f"[sol run] HARD_BUDGET=${HARD_BUDGET}  KILL_SWITCH=${KILL_SWITCH}")
    for model, p in PRICES.items():
        print(f"  {model}: input=${p['input']}  cached_input=${p['cached_input']}  output=${p['output']}")
    ledger = get_ledger()
    print(f"[sol run] Current recorded spend: ${ledger.total:.4f} ({ledger.state['n_calls']} calls)")


def chat_call(client, model, messages, max_completion_tokens, temperature=1.0, label="",
              response_format=None, reasoning_effort="none", max_retries=2):
    ledger = get_ledger()
    last_err = None
...
# ---------------------------------------------------------------------------
# Corpus: self written by Sol; other reused from the original study's gpt-4o-mini corpus
# ---------------------------------------------------------------------------
GEN_SCHEMA = {
    "type": "json_schema",
    "json_schema": {
...
def normalize_self_corpus(client: OpenAI) -> dict:
    ...
    for round_idx_str, msgs in raw.items():
        resp = chat_call(
            ...
            max_completion_tokens=1500, temperature=0.3, label=f"normalize:self:round{round_idx_str}",
...
def load_other_corpus() -> dict:
    """Reused from the original study (data/corpus_other.json, gpt-4o-mini), sliced to N_ROUNDS."""
...
def leakage_check(client: OpenAI, corpus_self: dict, corpus_other: dict) -> dict:
    if LEAKAGE_CHECK_PATH.exists():
        ...
        print(f"leakage_check_sol.json exists: accuracy={result['accuracy']:.3f} passed={result['passed']}")
        return result
    ...
    for i, item in enumerate(items):
        resp = chat_call(
            ...
            max_completion_tokens=20, temperature=1.0, label=f"leakage_check:{i}",
...
def get_noise_floor_prompts_sol() -> list[str]:
...
def analyze_and_write_results():
    ...
    lines = []
    lines.append("# Results: self/other boundary study -- gpt-5.6-sol smoke test")
    lines.append("")
    lines.append(f"Subject model: `{SUBJECT_MODEL}` (reasoning_effort='none' on every subject call).")
    lines.append(f'"Other" corpus messages are actually written by `{FOREIGN_CORPUS_ACTUAL_MODEL}` '
                 f'(reused from the original study\'s `data/corpus_other.json`); the "other" label '
                 f'sentence names a **mock competitor, "{FOREIGN_MODEL_LABEL_TEXT}"**, per the user\'s '
                 f'request -- this name does not correspond to any model actually queried in this run.')
    lines.append("")
...
def main():
    ...
    if not result["passed"]:
        print("!!! LEAKAGE CHECK FAILED !!! Stopping.", file=sys.stderr)
        print_prices()
        sys.exit(1)
    ...
    print("\n=== Step 4: noise floor ===")
    execute_tasks(client, build_task_list_noise_floor(), corpora, noise_floor_prompts=get_noise_floor_prompts_sol())
    print_prices()
